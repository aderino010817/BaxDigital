...
import MemberSidebar from "../Component/SidebarMember";
  ...
  return (
    <Flex>
      ...
      <Box w="80%" mt={"20px"} p={'3'} >
        ...
        <Text fontSize="lg" mb="4">Your Downline Members:</Text> 
        <List spacing={3}>
          {downlineMembers.map((member) => (
            <ListItem key={member.id} p={3} borderRadius={8} bg="white" boxShadow="md">
              <Flex justify="space-between" align="center">
                ...
                {member.level === 1 && <Badge colorScheme="green" fontSize="sm" py={1} px={2} borderRadius="md" marginRight="2">
                  <FaTrophy style={{ marginRight: '0.2em' }} /> Top Performer
                </Badge>}
                {member.level === 2 && <Badge colorScheme="blue" fontSize="sm" py={1} px={2} borderRadius="md" marginRight="2">
                  <FaStar style={{ marginRight: '0.2em' }} /> Silver Member
                </Badge>}
                {member.level > 2 && <Badge colorScheme="gray" fontSize="sm" py={1} px={2} borderRadius="md" marginRight="2">
